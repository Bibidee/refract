...
export function getClient(): GenLayerClient<GenLayerChain> {
  if (!_client) {
    const endpoint = rpc();
    _client = createClient({ chain: pickChain(), endpoint });
  }
  return _client;
}
...
export async function getClientReady(): Promise<GenLayerClient<GenLayerChain>> {
  if (typeof window !== "undefined" && window.ethereum) {
    ...
    const expected = "0x" + chainIdDecimal.toString(16);
...
export function setClientFromAddress(address: `0x${string}`) {
  const endpoint = rpc();
  _client = createClient({ chain: pickChain(), account: address, endpoint });
  return address;
}
